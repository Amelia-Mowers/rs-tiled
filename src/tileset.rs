...
use crate::{util::*, Gid, InvalidTilesetError, ResourceCache, Tile, TileId};
...
impl Tileset {
    ...
    fn parse_xml_reference(attrs: &[Attribute], map_path: &Path) -> Result<EmbeddedParseResult> {
...
fn parse_tileoffset(attrs: Vec<Attribute>) -> Result<(i32, i32)> {
    Ok(get_attrs!(
        for v in attrs {
